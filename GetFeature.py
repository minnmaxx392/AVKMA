    ...
    for section in pe.sections:
        ...
        for feature in Section:
            ...
            if isinstance(value, str):
                ...
                new_data.append(value)
    # Chèn các số 0 vào new_data nếu số lượng section không đủ section_limit
    if section_count < section_limit:
        remaining_sections = section_limit - section_count
...
def getImportEntry(pe):
    new_data = []
    imported_count = 0
    max_imported_libraries = 10 #default 5
    max_imported_functions = 15 #default 5
...
def getExport(pe):
    new_data = []
    function_name = []
    export = pe.exported_functions
    for func in export:
        value = func.name
        function_name.append(value)

    function_name_without_prefix = [value.replace("__", "") if "__" in value else value for value in
                                    function_name]

    for function in exportFunction:
        if function in function_name_without_prefix:
            new_data.append(1)
        else:
            new_data.append(0)
    return new_data
...
def create_raw_data(csv_file):
    ...
    with open(csv_file, mode='w', newline='') as file:
        ...
        writer.writerow(
            file_name + label + category_name + category_encoding + dos_header + header_characteristics_list + machine + header + optional_header + data_directory + section_name + total_section_name + import_data_directory + import_entry + tls_data + exportFunction)
...
if __name__ == "__main__":
    folder_path = r"D:\Data_test\50file"
    csv_file = r"data_raw_full_2516fts.csv"
